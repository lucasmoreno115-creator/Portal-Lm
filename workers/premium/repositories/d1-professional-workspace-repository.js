...
const OPERATIONAL_QUEUE_LIMIT = 12;
const OPERATIONAL_QUEUE_SELECT = `SELECT ib.id,ib.id student_id,ib.email,ib.name,ib.consultation_status,ib.access_status,ib.created_at,ib.whatsapp phone,ib.portal_access_status,ib.source,ib.identity_mode`;
  ...
  return Object.freeze({
    async getSummary({ now = new Date() } = {}) {
      ...
      const [pending, awaiting, missing, plan, anamnesis] = await Promise.all([
        db.prepare(IDENTITY_BRIDGE_CTE + ` SELECT COUNT(*) total FROM premium_pending_items pi JOIN identity_bridge ib ON pi.student_id=${sid} WHERE pi.status='OPEN' AND ib.consultation_status NOT IN ('PAUSED','ENDED')`).first(),
        db.prepare(IDENTITY_BRIDGE_CTE + ` SELECT COUNT(DISTINCT sc.id) total FROM student_checkins sc JOIN identity_bridge ib ON (sc.student_id=${sid} OR lower(trim(sc.student_email))=ib.normalized_email) WHERE ib.consultation_status NOT IN ('PAUSED','ENDED') AND NOT (${ANALYZED}) ${feedbackWeekFilter}`).bind(...feedbackParams).first(),
        ...
        db.prepare(IDENTITY_BRIDGE_CTE + ` SELECT COUNT(DISTINCT ib.id) total FROM identity_bridge ib WHERE ib.consultation_status NOT IN ('PAUSED','ENDED') AND EXISTS (SELECT 1 FROM premium_pending_items pi WHERE pi.student_id=${sid} AND pi.status='OPEN' AND pi.type='CREATE_NUTRITION_PLAN')`).first(),
        db.prepare(IDENTITY_BRIDGE_CTE + ` SELECT COUNT(DISTINCT pa.id) total FROM premium_anamnesis pa JOIN identity_bridge ib ON ${ANAMNESIS_MATCH.replaceAll('__SID__', sid)} WHERE ib.consultation_status NOT IN ('PAUSED','ENDED') AND upper(coalesce(pa.status,'')) NOT IN ('ANALYZED','ANALISADA')`).first(),
      ]);
    ...
    async getOperationalDashboard() {
      ...
      const [awaiting, received, ready, onboarding, underReview, readyToRelease, awaitingCheckins, backlogCheckins, pendingItems] = await Promise.all([
        ...
        db.prepare(IDENTITY_BRIDGE_CTE + ` ${OPERATIONAL_QUEUE_SELECT} FROM identity_bridge ib WHERE ib.consultation_status IN ('NEW','AWAITING_ANAMNESIS') ORDER BY datetime(ib.created_at) ASC, lower(coalesce(ib.name,ib.email)) ASC, ib.id ASC LIMIT ?`).bind(OPERATIONAL_QUEUE_LIMIT).all(),
        db.prepare(IDENTITY_BRIDGE_CTE + ` ${OPERATIONAL_QUEUE_SELECT},(SELECT MIN(coalesce(pa.created_at,pa.updated_at)) FROM premium_anamnesis pa WHERE ${ANAMNESIS_MATCH.replaceAll('__SID__', sid)}) anamnesis_recorded_at,CASE WHEN EXISTS(SELECT 1 FROM premium_anamnesis pa WHERE ${ANAMNESIS_MATCH.replaceAll('__SID__', sid)} AND upper(coalesce(pa.status,'')) IN ('ANALYZED','ANALISADA')) THEN 'ANALYZED' ELSE 'RESPONDED' END anamnesis_status FROM identity_bridge ib WHERE ib.consultation_status='UNDER_REVIEW' AND EXISTS (SELECT 1 FROM premium_anamnesis pa WHERE ${ANAMNESIS_MATCH.replaceAll('__SID__', sid)}) ORDER BY datetime(anamnesis_recorded_at) ASC, datetime(ib.created_at) ASC, lower(coalesce(ib.name,ib.email)) ASC, ib.id ASC LIMIT ?`).bind(OPERATIONAL_QUEUE_LIMIT).all(),
    ...
    async listPendingItems(filters = {}) {
      ...
      where.push("ib.consultation_status NOT IN ('PAUSED','ENDED')");
