...
function openPendingSelect(db, item) {
  return db.prepare(`SELECT * FROM premium_pending_items
    WHERE student_id=?
      AND type=?
      AND COALESCE(related_entity_type,'')=COALESCE(?,'')
      AND COALESCE(related_entity_id,'')=COALESCE(?,'')
      AND status='OPEN'
    LIMIT 1`).bind(item.student_id, item.type, item.related_entity_type ?? null, item.related_entity_id ?? null).first();
}
...
export function createD1PendingItemRepository(db) {
  return Object.freeze({
    async create(item) {
      ...
      await db.prepare(`INSERT OR IGNORE INTO premium_pending_items (
        id, student_id, type, title, description, status, priority, source,
        related_entity_type, related_entity_id, due_at, resolved_at, created_by, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
        item.id, item.student_id, item.type, item.title, item.description ?? null,
        item.status ?? 'OPEN', item.priority ?? 'NORMAL', item.source ?? 'manual',
        item.related_entity_type ?? null, item.related_entity_id ?? null, item.due_at ?? null,
        null, item.created_by ?? null, now, item.updated_at ?? now
      ).run();
      return openPendingSelect(db, item);
    },
    ...
    async resolveOpenByRelated({ student_id, type, related_entity_type, related_entity_id, resolved_at, created_by } = {}) { const now = resolved_at ?? new Date().toISOString(); await db.prepare(`UPDATE premium_pending_items SET status='RESOLVED', resolved_at=?, updated_at=?, created_by=COALESCE(created_by, ?) WHERE student_id=? AND type=? AND COALESCE(related_entity_type,'')=COALESCE(?,'') AND COALESCE(related_entity_id,'')=COALESCE(?,'') AND status='OPEN'`).bind(now, now, created_by ?? null, student_id, type, related_entity_type ?? null, related_entity_id ?? null).run(); return null; },
