...
export function createD1StudentRecordRepository(db) {
  return Object.freeze({
    ...
    async getNutritionPlanWorkflow(studentId) { const result = await db.prepare(`SELECT id, student_id, student_email, title, goal, strategy, status, version_number, published_at, updated_at, created_at FROM nutrition_plans WHERE student_id=? AND status IN ('PUBLISHED','DRAFT') ORDER BY CASE status WHEN 'DRAFT' THEN 0 ELSE 1 END, datetime(updated_at) DESC LIMIT 2`).bind(studentId).all(); const plans = rows(result); return { current: plans.find((plan) => plan.status === 'PUBLISHED') || null, draft: plans.find((plan) => plan.status === 'DRAFT') || null }; },
