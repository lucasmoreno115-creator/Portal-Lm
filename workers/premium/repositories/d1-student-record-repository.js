...
export function createD1StudentRecordRepository(db) {
  return Object.freeze({
    ...
    async getCurrentNutritionPlan(studentId) { const row = await db.prepare(`SELECT id, student_id, student_email, title, goal, strategy, meals_json, substitutions_json, adherence_rules_json, notes, whatsapp_message, updated_at, created_at FROM nutrition_plans WHERE student_id=? AND is_active=1 ORDER BY datetime(updated_at) DESC LIMIT 1`).bind(studentId).first(); return row ? { ...row, meals: parseJson(row.meals_json, []), substitutions: parseJson(row.substitutions_json, []), adherence_rules: parseJson(row.adherence_rules_json, []) } : null; },
