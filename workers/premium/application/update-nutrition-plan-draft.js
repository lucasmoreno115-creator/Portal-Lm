...
import { validateNutritionPlanDraftStructure } from '../domain/nutrition-plan-schema.js';
export function createUpdateNutritionPlanDraftUseCase({ nutritionPlanRepository }) {
  return { async execute({ id, student_id, updates }) {
    ...
    const validation = validateNutritionPlanDraftStructure(updates);
    if (!validation.ok) return { ok:false, error:'INVALID_DRAFT_STRUCTURE', details:validation.errors };
