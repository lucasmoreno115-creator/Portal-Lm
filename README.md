...
Normally you do not need to set `WRANGLER_BIN`. When it is not set, the tooling falls back to `npx wrangler` on Unix-like platforms and `npx.cmd wrangler` through `cmd.exe /d /s /c` on Windows.
...
SQL supplied to D1 is passed as a single `--command` argv value rather than concatenated into a shell command string, so spaces, quotes, and CMD metacharacters such as `&`, `|`, `(`, and `)` remain part of the SQL argument.
