...
test('minimal Workspace exposes only validated operational surface', async () => {
  ...
  for (const text of ['Workspace Premium', 'Buscar aluno', 'Atualizar', 'Sair', 'Alunos Premium', 'Contexto básico', 'Tentar novamente']) assert.match(html, new RegExp(text));
  for (const hidden of ['Inbox operacional', 'Revisão semanal', 'Filtros', 'Pendências', 'Plano Alimentar', 'Anamnese', 'Evolução', 'Prontuário', 'Feedback Semanal', 'Student 360']) assert.doesNotMatch(html, new RegExp(hidden));
});
