...
test('workspace UI contains the W2.1 operational surface without command center modules',()=>{const html=readFileSync('public/admin-premium-workspace.html','utf8');for(const pattern of [/<section class="workspace-dashboard" aria-labelledby="workspaceDashboardHeading">/,/<article class="workspace-dashboard-card" data-dashboard-card="anamnesis-pending">[\s\S]*?Aguardando anamnese/,/<article class="workspace-dashboard-card" data-dashboard-card="checkins-answered">[\s\S]*?Check-ins respondidos/,/<article(?=[^>]*class="[^"]*\bworkspace-dashboard-card\b[^"]*")(?=[^>]*data-dashboard-card="checkins-open")(?=[^>]*(?:aria-disabled="true"|class="[^"]*\bis-unavailable\b[^"]*"))[^>]*>[\s\S]*?Check-ins em aberto/,/<section id="students" class="panel">[\s\S]*?<h2>Alunos Premium<\/h2>/])assert.match(html,pattern);for(const text of ['Contexto básico','Selecione um aluno para visualizar o contexto básico','Inbox operacional','Revisão semanal','Filtros','Student 360'])assert.doesNotMatch(html,new RegExp(text));const js=readFileSync('public/admin-premium-workspace.js','utf8');for(const endpoint of ['/api/admin/premium/workspace/summary','/api/admin/premium/workspace/students','/api/admin/premium/workspace/pending-items'])assert.match(js,new RegExp(endpoint.replaceAll('/','\/')));assert.match(js,/diagnoseWorkspaceEndpoints/);assert.doesNotMatch(html,/<section id="record"\b/);assert.doesNotMatch(js,/loadSaturdayReview\(\)/);});
test('workspace exposes only the canonical detailed student action',()=>{const js=readFileSync('public/admin-premium-workspace.js','utf8');for(const text of ['Ver resumo','summaryButton','openRecord','renderRecord'])assert.doesNotMatch(js,new RegExp(text));assert.match(js,/Abrir Prontuário/);assert.match(js,/admin-premium-student-record\.html/);});
...
test('workspace feedback has semantic variants, accessible live regions, and synchronized runtime copies',()=>{
  ...
  for(const source of runtimes){
    ...
    assert.match(source,/showWorkspaceFeedback\(\{ type: 'success', message: 'Mensagem de acesso copiada com sucesso\.' \}\)/);
    assert.match(source,/function operationalWarning\(message\) \{ showWorkspaceFeedback\(\{ type: 'warning', message \}\); \}/);
