...
import {clsConsistency,expectedCls} from '../scripts/lib/portal-layout-stability.mjs';
...
test('CLS corresponde aos eventos drenados e mismatch é bloqueante',()=>{const events=[{value:.1,hadRecentInput:false},{value:.2,hadRecentInput:false},{value:.9,hadRecentInput:true}];assert.equal(expectedCls(events),.1+.2);assert.equal(clsConsistency(.1+.2,events).valid,true);assert.equal(clsConsistency(0,events).valid,false)});
test('coletor drena takeRecords, desconecta e captura métricas/eventos juntos',async()=>{const source=await readFile('scripts/measure-portal-performance.mjs','utf8');assert.match(source,/layout\.takeRecords\(\)/);assert.match(source,/observer\.disconnect\(\)/);assert.match(source,/CLS_EVENT_MISMATCH/);assert.match(source,/metrics:\{url:location\.href/);assert.doesNotMatch(source,/expression:'window\.__perf\?\.layoutShiftEvents/)});
...
test('LAB_STUBBED serves canonical Weekly Feedback current and keeps unknown APIs fail-closed',async()=>{const base=await mkdtemp(path.join(os.tmpdir(),'perf-weekly-feedback-'));const pub=path.join(base,'public');await mkdir(pub);const s=await startLocalServer(pub);try{const current=await fetch(`http://127.0.0.1:${s.port}/api/portal/premium/weekly-feedback/current`);assert.equal(current.status,200);const body=await current.json();assert.equal(body.ok,true);assert.equal(body.data.status,'AVAILABLE');assert.equal(body.data.professionalResponse,null);const unknown=await fetch(`http://127.0.0.1:${s.port}/api/portal/premium/not-defined`);assert.equal(unknown.status,404);assert.deepEqual(await unknown.json(),{ok:false,error:'LAB_STUB_NOT_DEFINED'})}finally{await s.close();await rm(base,{recursive:true,force:true})}});
