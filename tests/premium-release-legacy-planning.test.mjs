...
test('falha otimista não persiste acesso, plano nem auditoria', async () => {
  const state = { optimisticZero:true, access:{id:'a1',student_id:'s1',email:'ana@example.com',status:'INACTIVE',plan:'premium'}, premium:{student_id:'s1',email:'ana@example.com',consultation_status:'UNDER_REVIEW',access_status:'ACTIVE'}, plan:{id:'p1',student_id:null,student_email:'ana@example.com',status:'PUBLISHED',is_active:1}, entries:[] };
  const result = await createReleaseLegacyPlanningUseCase({db:fakeDb(state),randomUUID:()=> 'audit'})({student_id:'s1'});
  assert.equal(result.status, 409); assert.equal(state.access.status, 'INACTIVE'); assert.equal(state.plan.student_id, null); assert.equal(state.entries.length, 0);
});
...
function fakeDb(s) { return { prepare(sql) { let params=[]; return { bind(...v) { params=v; return this; }, async all() { if (sql.includes('FROM student_access WHERE student_id')) return {results:s.access?.student_id===params[0]?[s.access]:[]}; if (sql.includes('FROM premium_students WHERE student_id')) return {results:s.premium?.student_id===params[0]?[s.premium]:[]}; if (sql.includes('FROM nutrition_plans')) return {results:s.plan.status==='PUBLISHED'&&s.plan.is_active===1&&s.plan.student_id===params[0]||s.plan.status==='PUBLISHED'&&s.plan.is_active===1&&s.plan.student_id==null?[s.plan]:[]}; return {results:[]}; }, async run() { if (sql.startsWith('INSERT INTO premium_students')) s.premium={student_id:params[0],email:params[1],normalized_email:params[2],display_name:params[3],consultation_status:'ACTIVE',access_status:'ACTIVE',source:'LEGACY_IMPORT'}; else if (sql.startsWith('UPDATE premium_students')) { if (s.optimisticZero) return {meta:{changes:0}}; s.premium.consultation_status='ACTIVE';s.premium.access_status='ACTIVE'; } else if (sql.startsWith('UPDATE student_access')) s.access.status='ACTIVE'; else if (sql.startsWith('UPDATE nutrition_plans')) s.plan.student_id=params[0]; else if (sql.startsWith('INSERT INTO premium_followup_entries')) s.entries.push({content:params[2]}); return {meta:{changes:1}}; } }; }, async batch(statements) { return Promise.all(statements.map(x=>x.run())); } }; }
