...
test('Workspace uses one centralized summary request to render cards and operational panels', async () => {
  ...
  assert.match(source, /function renderSummary\(data\) \{[\s\S]*?renderDashboardCards\(data\);[\s\S]*?renderPendingItems\(pendingItems\);[\s\S]*?anamnesisDashboard\.append[\s\S]*?checkinDashboard\.append/);
...
test('Workspace maps official summary fields to the three dashboard cards', async () => {
  ...
  assert.match(source, /'pendingItemsValue', 'pendingItemsHint'[\s\S]*?pendingItems\.open/);
  assert.match(source, /pendingItems\.high > 0 \? `\$\{pendingItems\.high\} de alta prioridade` : 'Nenhuma de alta prioridade'/);
  assert.doesNotMatch(source, /checkinsOpenValue/);
});
...
test('Workspace markup provides stable identifiers for accessible card updates', async () => {
  ...
  for (const id of ['workspaceDashboard', 'anamnesisPendingValue', 'anamnesisPendingHint', 'checkinsAnsweredValue', 'checkinsAnsweredHint', 'pendingItemsValue', 'pendingItemsHint']) assert.match(html, new RegExp(`id="${id}"`));
});
