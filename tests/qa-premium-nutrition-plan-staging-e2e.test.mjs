...
// Exact legacy publicNutritionPlan() shape consumed by portal-plano-alimentar.html.
const publicMeals=[{id:'meal-1',name:'Café da manhã',guidance:editedMarker,items:[{food:`Aveia ${marker}`,quantity:'40',unit:'g'},{food:`Banana ${marker}`,quantity:'1',unit:'unidade'}]},{id:'meal-2',name:'Almoço',primary_text:marker,items:[]}];
const publicPlan={id:planId,student_email:'student@example.test',title:`Plano ${marker}`,goal:'',strategy:'',meals:publicMeals,substitutions:[],adherence_rules:[],notes:marker,whatsapp_message:'',created_at:'2026-08-10T00:00:00Z',updated_at:'2026-08-10T01:00:00Z'};
const validatePublic=(body={ok:true,data:publicPlan},status=200)=>validateStudentPlan(result(body,status),{planId,marker,editedMarker,deletedMarker});
test('real public response without status and version_number passes',()=>{assert.equal('status' in publicPlan,false);assert.equal('version_number' in publicPlan,false);assert.equal(validatePublic().ok,true);});
test('post-release HTTP 403 fails the public contract',()=>assert.equal(validatePublic({ok:false},403).ok,false));
test('post-release HTTP 4xx fails the public contract',()=>assert.equal(validatePublic({ok:false},404).ok,false));
test('HTTP 200 with ok false fails the public contract',()=>assert.equal(validatePublic({ok:false,data:publicPlan}).ok,false));
test('missing public data fails closed',()=>assert.equal(validatePublic({ok:true}).ok,false));
test('missing public meals fails closed',()=>{const {meals:_,...withoutMeals}=publicPlan;assert.equal(validatePublic({ok:true,data:withoutMeals}).ok,false);});
test('missing run marker fails content consistency',()=>assert.equal(validatePublic({ok:true,data:{...publicPlan,title:'Outro plano',notes:'',meals:publicMeals.map(meal=>({...meal,primary_text:'',items:[]}))}}).ok,false));
test('missing edited marker fails content consistency',()=>assert.equal(validatePublic({ok:true,data:{...publicPlan,meals:publicMeals.map(meal=>({...meal,guidance:''}))}}).ok,false));
test('deleted marker in public content fails content consistency',()=>assert.equal(validatePublic({ok:true,data:{...publicPlan,notes:deletedMarker}}).ok,false));
test('content from another run fails content consistency',()=>assert.equal(validatePublic({ok:true,data:{...publicPlan,title:'QA-F1.8-other',notes:'QA-F1.8-other',meals:[{name:'Café',guidance:'QA-F1.8-other-EDITADO',items:[{food:'A'},{food:'B'}]}]}}).ok,false));
test('structured draft metadata exposed publicly fails closed without textual false positives',()=>{assert.equal(validatePublic({ok:true,data:{...publicPlan,draft:{id:'draft-1'}}}).ok,false);assert.equal(validatePublic({ok:true,data:{...publicPlan,notes:`Texto clínico contém DRAFT, ${marker}`}}).ok,true);});
test('administrative status and version are not required by the public validator',()=>assert.equal(validatePublic().contractOk,true));
test('expected published version content and its two structured items pass',()=>{const validation=validatePublic();assert.equal(validation.itemCount,2);assert.equal(validation.markerPresent,true);assert.equal(validation.editedMarkerPresent,true);assert.equal(validation.deletedMarkerPresent,false);assert.equal(validation.ok,true);});
test('a different public plan id cannot be correlated to the professional current',()=>assert.equal(validatePublic({ok:true,data:{...publicPlan,id:'other-plan'}}).ok,false));
