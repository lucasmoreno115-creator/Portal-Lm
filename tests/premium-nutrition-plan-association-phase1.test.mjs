...
function client({ changes = 1, audit = true, mutate = true, altered = false } = {}) {
  const sql = []; const state = { plan: { ...plan }, recordStatus: audit ? 'APPLIED' : null, recordFingerprint: null };
  return { sql, state, async all(query, params = []) {
    ...
    if (query.includes('WHERE id = ?')) return [state.plan];
    return [];
    ...
    if (query.startsWith('UPDATE nutrition_plans')) { if (mutate && changes === 1) state.plan.student_id = params[0]; return { meta: { changes } }; }
    if (query.includes("SET status = 'APPLIED'")) state.recordStatus = 'APPLIED';
    if (query.includes("SET status = 'ROLLED_BACK'")) state.recordStatus = 'ROLLED_BACK';
