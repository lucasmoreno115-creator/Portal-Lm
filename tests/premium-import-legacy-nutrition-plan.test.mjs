...
import { executeSql, querySql, SqliteD1 } from './helpers/sqlite-d1.mjs';
...
test('importa snapshot publicado, preservando legado e DRAFT, e é idempotente', async () => {
  ...
  const imported=state.plans[2]; assert.equal(imported.status,'PUBLISHED'); assert.equal(imported.is_active,1); assert.equal(imported.student_email,'ana@example.com'); assert.equal(imported.meals_json,legacy.meals_json); assert.equal(imported.substitutions_json,legacy.substitutions_json); assert.equal(imported.supersedes_plan_id,legacy.id); assert.deepEqual(JSON.parse(imported.private_notes),{origin:'LEGACY_IMPORT',legacy_plan_id:legacy.id,legacy_student_email:legacy.student_email,legacy_private_notes:legacy.private_notes});
...
test('formato legado pós e pré migration é elegível, sem ampliar status não comprovados', async () => {
  ...
  assert.doesNotMatch(originalSchema,/\bstatus\b/i); assert.match(lifecycle,/ADD COLUMN status TEXT/); assert.match(backfill,/WHERE status IS NULL/);
  for (const status of ['PUBLISHED', null]) await withSqliteD1(async ({file,db}) => { seedRealLegacy(file, { id: `legacy-${status || 'null'}`, status }); const result = await createImportLegacyNutritionPlanUseCase({ db, randomUUID: () => 'import-real' })({ student_id: 'student-real' }); assert.equal(result.ok, true); });
  for (const status of ['', 'ACTIVE']) await withSqliteD1(async ({file,db}) => { seedRealLegacy(file, { id: `legacy-${status || 'empty'}`, status }); const result = await createImportLegacyNutritionPlanUseCase({ db, randomUUID: () => 'import-real' })({ student_id: 'student-real' }); assert.equal(result.status, 404); });
});
test('db.batch D1/SQLite faz rollback real quando a auditoria falha após inserir o snapshot', async () => {
  await withSqliteD1(async ({file,db}) => { seedRealLegacy(file); executeSql(file, `CREATE TRIGGER fail_legacy_import_audit BEFORE INSERT ON premium_followup_entries WHEN NEW.title='Planejamento antigo importado' BEGIN SELECT RAISE(ABORT, 'AUDIT_FAILURE'); END;`); const result = await createImportLegacyNutritionPlanUseCase({ db, randomUUID: () => 'import-real', now: () => '2026-07-22T00:00:00.000Z' })({ student_id: 'student-real' }); assert.equal(result.status, 409); const counts=querySql(file, "SELECT (SELECT COUNT(*) FROM nutrition_plans WHERE student_id='student-real' AND status='PUBLISHED') AS snapshots, (SELECT COUNT(*) FROM premium_followup_entries WHERE student_id='student-real') AS audits, (SELECT status FROM nutrition_plans WHERE id='legacy-real') AS legacy_status")[0]; assert.deepEqual(counts,{snapshots:0,audits:0,legacy_status:null}); });
});

test('Student Record sinaliza legado PUBLISHED ativo e não associado', async () => {
  await withSqliteD1(async ({file,db}) => { seedRealLegacy(file, { status: 'PUBLISHED' }); const workflow=await createD1StudentRecordRepository(db).getNutritionPlanWorkflow('student-real'); assert.equal(workflow.legacy_available,true); });
});
