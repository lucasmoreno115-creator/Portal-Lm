...
test('V5 journey contract exposes frontend-ready shape, progress, statuses and next action', async () => {
  ...
  assert.equal(initial.body.data.view_model.progress_label, 'Comece pelas três ações mínimas.');
  assert.equal(initial.body.data.view_model.primary_message, 'Escolha 3 ações simples para fazer nesta semana.');
  assert.deepEqual(initial.body.data.view_model.primary_cta, { label: 'Definir minhas 3 ações', action: 'open_stage_1_actions' });
  ...
  assert.deepEqual(initial.body.data.view_model.stage_cards[0].cta, { label: 'Marcar ações concluídas', action: 'open_stage_1_actions' });
  assert.equal(initial.body.data.view_model.stage_cards[0].progress_text, '0/3 ações concluídas');
  assert.equal(initial.body.data.view_model.stage_cards[0].empty_state, 'Defina suas 3 ações mínimas para começar.');
  assert.equal(initial.body.data.view_model.stage_cards[1].cta, null);
  assert.equal(initial.body.data.view_model.stage_cards[2].empty_state, 'Registre uma escolha concreta de hoje.');
  assert.equal(initial.body.data.view_model.stage_cards[4].cta, null);
  assert.equal(initial.body.data.view_model.stage_cards[4].empty_state, 'Defina uma meta simples para os próximos dias.');
  ...
  assert.equal(stage1.body.data.view_model.primary_message, 'Conclua suas ações mínimas pendentes.');
  assert.deepEqual(stage1.body.data.view_model.primary_cta, { label: 'Marcar ações concluídas', action: 'open_stage_1_actions' });
  ...
  assert.equal(firstCompleted.body.data.view_model.progress_label, 'Continue concluindo suas ações mínimas.');
  ...
  assert.equal(stage2.body.data.view_model.progress_label, 'Crie seu Plano B.');
  assert.deepEqual(stage2.body.data.view_model.primary_cta, { label: 'Criar meu Plano B', action: 'open_plan_b' });
  ...
  assert.deepEqual(stage2.body.data.view_model.stage_cards[1].cta, { label: 'Criar meu Plano B', action: 'open_plan_b' });
  ...
  assert.equal(stage3.body.data.view_model.progress_label, 'Registre vitórias concretas.');
  ...
  assert.equal(stage4.body.data.view_model.progress_label, 'Defina seus protocolos de recuperação.');
  assert.deepEqual(stage4.body.data.view_model.primary_cta, { label: 'Definir protocolos de recuperação', action: 'open_recovery_protocols' });
  ...
  assert.equal(maintenance.body.data.view_model.progress_label, 'Você está em manutenção.');
  assert.equal(maintenance.body.data.view_model.primary_message, 'Defina uma meta simples para sustentar sua rotina.');
