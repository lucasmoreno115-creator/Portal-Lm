...
export function auditNutritionPlanLifecycle({ nutrition_plans = [], premium_students = [], student_access = [] } = {}) {
  ...
  for (const plan of nutrition_plans) {
    if (Number(plan.is_active) === 1 && plan.student_id) activeByStudent.set(plan.student_id, [...(activeByStudent.get(plan.student_id) || []), plan.id]);
    if (Number(plan.is_active) === 1 && !plan.student_id) conflicts.push({ type: 'ACTIVE_WITHOUT_STUDENT_ID', plan_id: plan.id, student_email: plan.student_email });
    ...
    if (plan.student_id && premiumIds.size && !premiumIds.has(plan.student_id)) conflicts.push({ type: 'ACTIVE_NON_PREMIUM_STUDENT', plan_id: plan.id, student_id: plan.student_id });
    if ((plan.status === 'ARCHIVED' || plan.status === 'DRAFT') && Number(plan.is_active) === 1) conflicts.push({ type: 'STATUS_IS_ACTIVE_MISMATCH', plan_id: plan.id, status: plan.status, is_active: plan.is_active });
    if (plan.status === 'PUBLISHED' && Number(plan.is_active) !== 1) conflicts.push({ type: 'STATUS_IS_ACTIVE_MISMATCH', plan_id: plan.id, status: plan.status, is_active: plan.is_active });
  }
