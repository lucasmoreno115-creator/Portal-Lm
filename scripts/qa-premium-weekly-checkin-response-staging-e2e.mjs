...
export async function runPremiumWeeklyCheckinResponseSmoke({env=process.env,requestFn,mask=()=>{}}={}) {
  ...
  const run=env.GITHUB_RUN_ID||Date.now(), attempt=env.GITHUB_RUN_ATTEMPT||'1', checkinMarker=`QA-F2.1-CHECKIN-${run}-${attempt}`, responseMarker=`QA-F2.1-RESPONSE-${run}-${attempt}`;
  ...
  const anamnesis=await call('/api/anamnese-premium',{method:'POST',headers:sh,expectedStatus:[200],body:{answers:createQaAnswers(checkinMarker)}});
  const objectives=createObjectives(checkinMarker), objectivesResult=await call(`/api/admin/premium/students/${encodeURIComponent(studentId)}/planning-objectives`,{method:'PUT',headers:ah,expectedStatus:[200],body:objectives});
  ...
  const draft=data(await call(`/api/admin/premium/students/${encodeURIComponent(studentId)}/nutrition-plan/draft`,{method:'POST',headers:ah,expectedStatus:[200],body:{plan:{title:`Plano ${checkinMarker}`,meals:[],substitutions:[]}}}));
  const written=await call(`/api/admin/premium/nutrition-plans/${encodeURIComponent(draft?.id)}/draft`,{method:'PATCH',headers:ah,expectedStatus:[200],body:{student_id:studentId,title:`Plano ${checkinMarker}`,meals:[{name:'Plano base',primary_text:checkinMarker,items:[],substitutions:[]}],substitutions:[],adherence_rules:[],notes:checkinMarker,expected_updated_at:draft?.updated_at}});
  ...
  const initial=data(await call(recordPath,{headers:ah,expectedStatus:[200]})), initiallyAbsent=!JSON.stringify(initial||{}).includes(checkinMarker);
  add('checkin-initial-state','marker da execução ainda não existe no Prontuário',{markerPresent:!initiallyAbsent},initiallyAbsent); if(!initiallyAbsent)return finish();
