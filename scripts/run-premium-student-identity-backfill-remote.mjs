...
import { normalizePremiumStudentEmail } from '../workers/premium/services/student-identity-service.js';

const MODES = new Set(['dry-run', 'apply', 'rollback']);
const REQUEST_TIMEOUT_MS = 30_000;
const BATCH_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/;
const NON_BLOCKING_CONFLICT_TYPES = new Set(['INVALID_EMAIL', 'INACTIVE_ACCESS', 'NON_PREMIUM_ACCESS', 'PREMIUM_DATA_WITHOUT_ACCESS']);
...
function emailHash(email) {
  return createHash('sha256').update(`legacy-premium-backfill-v1:${email}`).digest('hex');
}

function candidateIds(conflict) {
  const sourceIds = Array.isArray(conflict.student_access_ids) && conflict.student_access_ids.length > 0
    ? conflict.student_access_ids
    : String(conflict.record ?? '').split(',');
  return sourceIds.map((id) => String(id).trim()).filter(Boolean);
}

// This report is intentionally restricted to operational metadata. Raw e-mail,
// name, token, and clinical fields never leave this process.
export async function createRestrictedBlockerReport({ repository, result }) {
  ...
  for (const candidate of candidates) {
    try {
      const normalized = normalizePremiumStudentEmail(candidate.email, { required: true });
      byEmail.set(normalized, [...(byEmail.get(normalized) ?? []), candidate]);
    } catch { /* Invalid e-mail is excluded from the restricted ambiguity report. */ }
  }
  const reports = [];
  for (const conflict of result.conflicts ?? []) {
    if (NON_BLOCKING_CONFLICT_TYPES.has(conflict.type)) continue;
    const related = candidateIds(conflict).map((id) => byId.get(id)).filter(Boolean);
    const representative = related[0] ?? null;
    let normalizedEmail = null;
    try { normalizedEmail = representative ? normalizePremiumStudentEmail(representative.email, { required: true }) : null; } catch { /* no identifier */ }
    const sameEmail = normalizedEmail ? byEmail.get(normalizedEmail) ?? [] : [];
    const identities = normalizedEmail ? await repository.findByNormalizedEmail(normalizedEmail) : [];
    reports.push({
      type: conflict.type,
      student_access_ids: related.map((candidate) => String(candidate.id)),
      email_hash: normalizedEmail ? emailHash(normalizedEmail) : null,
      plan: representative?.plan ?? null,
      plan_type: representative?.plan_type ?? null,
      same_email_record_count: sameEmail.length,
      premium_identity_count: Array.isArray(identities) ? identities.length : Number(Boolean(identities)),
...
export async function executeRemoteBackfill({ mode, batchId = '', confirmApply = '', confirmRollback = '', env = process.env, fetchImpl, timeoutMs } = {}) {
  ...
  if (mode === 'dry-run') return { ...drySummary, restricted_blockers: await createRestrictedBlockerReport({ repository, result: dryRun }) };
