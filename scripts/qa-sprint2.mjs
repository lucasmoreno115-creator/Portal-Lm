...
function assert(condition, scope, message, details = {}) {
  if (condition) pass(scope, message, details);
  else fail(scope, message, details);
}
...
function auditDatabaseContract() {
  ...
  try {
    ...
    const requiredColumns = [
      'id', 'student_id', 'student_email', 'title', 'meals_json', 'substitutions_json',
      'adherence_rules_json', 'status', 'version_number', 'published_at', 'published_by',
      'archived_at', 'supersedes_plan_id', 'is_active', 'created_at', 'updated_at',
    ];
    ...
    const schemaSql = String(table.sql).toUpperCase();
    for (const status of ['DRAFT', 'PUBLISHED', 'ARCHIVED']) {
      assert(schemaSql.includes(status), scope, `Status ${status} está protegido pelo schema.`);
    }
...
function auditProductionCodeContracts() {
  ...
  for (const file of requiredFiles) assert(existsSync(path.join(root, file)), scope, `Arquivo obrigatório presente: ${file}`);
  if (failures.some((item) => item.scope === scope)) return;

  const repository = read('workers/premium/repositories/d1-nutrition-plan-repository.js');
  const publishUseCase = read('workers/premium/application/publish-nutrition-plan.js');
  const updateUseCase = read('workers/premium/application/update-nutrition-plan-draft.js');
  const worker = read('workers/api.js');
  const publicPresenter = read('workers/premium/presenters/nutrition-plan-public-presenter.js');
  ...
  const contracts = [
    ...
    [repository.includes("entry_type = 'PLAN_CHANGE'") || repository.includes("'PLAN_CHANGE'"), 'Publicação registra mudança no histórico operacional.'],
    ...
    [publicPresenter.includes('meals') && publicPresenter.includes('substitutions') && publicPresenter.includes('adherence'), 'Presenter público preserva refeições, substituições e regras de adesão.'],
  ];
...
function createMemoryRepositories() {
  ...
  const plans = [];
  let clock = 0;
  const now = () => `2026-07-22T14:00:${String(clock++).padStart(2, '0')}Z`;

  const studentRepository = {
    async findByStudentId(studentId) { return studentId === student.student_id ? student : null; },
  };
  ...
  const nutritionPlanRepository = {
    ...
    async createDraft(plan) {
      ...
      const timestamp = now();
      const draft = { ...plan, status: 'DRAFT', version_number: null, is_active: 0, created_at: timestamp, updated_at: timestamp };
      plans.push(draft);
      return draft;
    },
    async updateDraft(id, updates) {
      const plan = await this.findById(id);
      if (!plan || plan.status !== 'DRAFT') throw Object.assign(new Error('NOT_DRAFT'), { conflict: true });
      if (plan.updated_at !== updates.expected_updated_at) throw Object.assign(new Error('NUTRITION_PLAN_DRAFT_CONFLICT'), { conflict: true });
    ...
    async publish(id, { published_by = 'admin' } = {}) {
      ...
      const previous = plans.find((item) => item.student_id === plan.student_id && item.status === 'PUBLISHED' && item.is_active === 1);
      if (previous) Object.assign(previous, { status: 'ARCHIVED', is_active: 0, archived_at: now(), updated_at: now() });
      const version = Math.max(0, ...plans.filter((item) => item.student_id === plan.student_id).map((item) => Number(item.version_number || 0))) + 1;
      Object.assign(plan, { status: 'PUBLISHED', is_active: 1, version_number: version, published_at: now(), published_by, supersedes_plan_id: previous?.id ?? null, updated_at: now() });
      return plan;
    },
...
async function auditApplicationFlow() {
  ...
  let idSequence = 1;
  const randomUUID = () => `nutrition-qa-${idSequence++}`;
  ...
  const publishPlan = createPublishNutritionPlanUseCase({ nutritionPlanRepository, randomUUID });
  ...
  const planContent = {
    title: 'Plano QA 2',
    goal: 'Validar fluxo',
    strategy: 'Consistência',
    meals: [{ id: 'meal-1', name: 'Café da manhã', primary_text: '2 ovos e 1 fruta', items: [], substitutions: [{ id: 'sub-1', text: 'Trocar a fruta por outra porção equivalente' }] }],
    substitutions: [],
    adherenceRules: ['Seguir a estrutura publicada'],
    notes: 'Plano de diagnóstico',
  };

  const created = await createDraft.execute({ student_id: student.student_id, plan: planContent });
  assert(created.ok && created.data?.status === 'DRAFT' && Number(created.data?.is_active) === 0, scope, 'Criação gera rascunho inativo.');

  const loadedDraft = await getDraft.execute({ student_id: student.student_id });
  assert(loadedDraft.ok && loadedDraft.data?.id === created.data?.id, scope, 'Leitura de rascunho retorna o mesmo plano.');

  const missingRevision = await updateDraft.execute({ id: created.data.id, student_id: student.student_id, updates: { ...planContent, title: 'Sem revisão' } });
  assert(!missingRevision.ok && missingRevision.error === 'REVISION_REQUIRED', scope, 'Salvar sem revisão é bloqueado.');

  const updated = await updateDraft.execute({ id: created.data.id, student_id: student.student_id, updates: { ...planContent, title: 'Plano QA 2 atualizado', expected_updated_at: created.data.updated_at } });
  assert(updated.ok && updated.data?.title === 'Plano QA 2 atualizado' && updated.data?.status === 'DRAFT', scope, 'Rascunho é atualizado sem alterar o status.');

  const publishedV1 = await publishPlan.execute({ id: created.data.id, student_id: student.student_id, published_by: 'qa-agent' });
  assert(publishedV1.ok && publishedV1.data?.status === 'PUBLISHED' && publishedV1.data?.is_active === 1 && publishedV1.data?.version_number === 1, scope, 'Primeira publicação cria versão ativa 1.');

  const currentV1 = await getCurrent.execute({ student_id: student.student_id });
  assert(currentV1.ok && currentV1.data?.id === publishedV1.data?.id, scope, 'Portal recebe a versão publicada ativa.');

  const draftV2 = await createDraft.execute({ student_id: student.student_id, plan: { ...planContent, title: 'Plano QA 2 versão 2' } });
  assert(draftV2.ok && draftV2.data?.status === 'DRAFT', scope, 'Novo ciclo cria um novo rascunho após publicação.');

  const currentBeforeV2 = await getCurrent.execute({ student_id: student.student_id });
  assert(currentBeforeV2.data?.id === publishedV1.data?.id, scope, 'Rascunho novo não altera o plano visível ao aluno.');

  const publishedV2 = await publishPlan.execute({ id: draftV2.data.id, student_id: student.student_id, published_by: 'qa-agent' });
  const archivedV1 = plans.find((plan) => plan.id === publishedV1.data.id);
  assert(publishedV2.ok && publishedV2.data?.version_number === 2 && publishedV2.data?.supersedes_plan_id === publishedV1.data.id, scope, 'Segunda publicação incrementa versão e referencia a anterior.');
  assert(archivedV1?.status === 'ARCHIVED' && archivedV1?.is_active === 0, scope, 'Versão anterior é arquivada e desativada.');

  const currentV2 = await getCurrent.execute({ student_id: student.student_id });
  assert(currentV2.data?.id === publishedV2.data.id && currentV2.data?.version_number === 2, scope, 'Portal passa a consumir somente a versão 2.');

  const history = await getHistory.execute({ student_id: student.student_id });
  assert(history.ok && history.data?.length === 2 && history.data[0]?.version_number === 2 && history.data[1]?.status === 'ARCHIVED', scope, 'Histórico preserva e ordena as versões publicadas.');

  const foreignPublish = await publishPlan.execute({ id: publishedV2.data.id, student_id: 'outro-aluno', published_by: 'qa-agent' });
  assert(!foreignPublish.ok && foreignPublish.error === 'NOT_FOUND', scope, 'Publicação por student_id divergente é bloqueada.');
}
