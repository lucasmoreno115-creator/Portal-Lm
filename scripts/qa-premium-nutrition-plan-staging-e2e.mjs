...
export async function runPremiumNutritionPlanSmoke({ env=process.env, requestFn, mask=()=>{} }={}) {
  ...
  add('student-plan-read','sem auth é 401; aluno autenticado recebe a versão recém-publicada',{unauthenticatedStatus:unauth.status,httpStatus:portal.status},current.ok&&unauth.status===401&&studentPlan.ok?'PASSED':'FAILED');
