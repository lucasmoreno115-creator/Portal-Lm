...
export function validateDraft(result, studentId) { const body=parse(result), plan=body?.data; return { ok:result.ok&&result.status===200&&body?.ok===true&&!invalidId(plan?.id)&&plan?.student_id===studentId&&plan?.status==='DRAFT'&&Number(plan?.is_active)===0, planId:plan?.id||null, plan }; }
...
export function validatePublish(result, { planId, studentId }) { const body=parse(result), plan=body?.data; return { ok:result.ok&&result.status===200&&body?.ok===true&&plan?.id===planId&&plan?.student_id===studentId&&plan?.status==='PUBLISHED'&&Number(plan?.is_active)===1&&Number(plan?.version_number)>0, plan }; }
export function validateStudentPlan(result, { marker, editedMarker, deletedMarker }) { const body=parse(result), plan=body?.data, serialized=JSON.stringify(plan||{}); return { ok:result.ok&&result.status===200&&body?.ok===true&&plan?.status==='PUBLISHED'&&serialized.includes(marker)&&serialized.includes(editedMarker)&&!serialized.includes(deletedMarker)&&!serialized.includes('DRAFT'), plan }; }
...
export async function runPremiumNutritionPlanSmoke({ env=process.env, requestFn, mask=()=>{} }={}) {
  ...
  add('draft-create','rascunho inativo com ID canônico',{httpStatus:draftResult.status,planId:draft.planId,status:draft.plan?.status||null},draft.ok?'PASSED':'FAILED');if(!draft.ok)return finish();
  const mealPayload=[
  ...
  const reload1=await call(endpoint,{headers:adminHeaders,expectedStatus:[200]}), reloadDraft=data(reload1)?.draft, persistenceOk=reload1.ok&&reloadDraft?.id===draft.planId&&reloadDraft?.status==='DRAFT'&&JSON.stringify(reloadDraft.meals).includes(marker)&&reloadDraft.meals?.length===3;
  add('draft-persistence','leitura independente mantém versão, refeições e itens',{httpStatus:reload1.status,planId:reloadDraft?.id||null,mealCount:reloadDraft?.meals?.length||0},persistenceOk?'PASSED':'FAILED');if(!persistenceOk)return finish();
  ...
  add('publish','publicação confirma status, ativação e versão',{httpStatus:publishResult.status,planId:published.plan?.id||null,status:published.plan?.status||null,version:published.plan?.version_number||null},published.ok?'PASSED':'FAILED');if(!published.ok)return finish();
  const professional=await call(endpoint,{headers:adminHeaders,expectedStatus:[200]}), pro=data(professional), proOk=professional.ok&&pro?.current?.id===draft.planId&&pro.current.status==='PUBLISHED'&&pro.draft==null&&JSON.stringify(pro.current.meals).includes(editedMarker);
  add('professional-reload','leitura profissional mostra somente versão publicada correta',{httpStatus:professional.status,currentId:pro?.current?.id||null,draftPresent:Boolean(pro?.draft)},proOk?'PASSED':'FAILED');
  const unauth=await call('/api/portal/nutrition-plan',{expectedStatus:[401]});
  const portal=await call('/api/portal/nutrition-plan',{headers:studentHeaders,expectedStatus:[200]}), studentPlan=validateStudentPlan(portal,{marker,editedMarker,deletedMarker});
  add('student-plan-read','sem auth é 401; aluno autenticado recebe HTTP 200',{unauthenticatedStatus:unauth.status,httpStatus:portal.status},unauth.status===401&&studentPlan.ok?'PASSED':'FAILED');
