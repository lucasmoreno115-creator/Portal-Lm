...
The manifest records the historical migration hash, override hash, reason, allowed environments, semantic effect, and `productionAllowed: false`. Before any override executes, the tool recomputes SHA-256 for both the historical migration and override file and compares actual values with the manifest. During replay, the tool verifies that the 0021 columns already exist from 0019, executes the override, records the historical migration as semantically satisfied, and continues with later migrations. Without the override, the duplicate-column conflict remains detectable and blocking. If any hash, required manifest field, environment allowlist, production prohibition, file existence check, or semantic condition fails, replay fails closed with `BLOCKING`, the override is not executed, later migrations are not executed, and expected-schema/catalog generation is not updated.
