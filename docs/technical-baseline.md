...
API inventory recognizes literal routes passed to `fetch`, Axios, and the local `api()` wrapper. Wrapper routes beginning with `/portal/` are reported under their effective `/api/portal/` path, and dynamic template segments are represented as `{dynamic}`. Service Worker precache inventory resolves simple string constants; unresolved expressions produce a `PARTIAL` status and an aggregate `unresolvedEntries` entry without copying source code into the report.
