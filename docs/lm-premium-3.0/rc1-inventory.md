...
## Atualização pós-comentário PR #270
- Auditoria RC1 operacional: aceita somente `--snapshot ./snapshot.json` nesta versão.
- Contrato mínimo do snapshot: `premium_students`, `student_access`, `premium_anamnesis`, `student_checkins`, `nutrition_plans`, `premium_pending_items`, `premium_followup_entries`, `activity_timeline`.
- Smoke RC1 operacional: aceita `--base-url`, `--admin-token`, `--student-token`, `--student-id`, `--timeout-ms` e `--dry-run`.
- Verify RC1 operacional: aceita `--snapshot`, `--schema`, `--smoke-results` e `--flags`.
- Evidência pendente: execução de staging com D1 real, autenticação real e restore testado.
